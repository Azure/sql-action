...
describe('SqlUtils tests', () => {
    it('detectIPAddress should return ipaddress', async () => {
        ...
        const ipAddress = await SqlUtils.detectIPAddress(new SqlConnectionStringBuilder('Server=testServer.database.windows.net;Initial Catalog=testDB;User Id=testUser;Password=placeholder'));
    ...
    it('detectIPAddress should throw error', () => {
        ...
        expect(SqlUtils.detectIPAddress(new SqlConnectionStringBuilder('Server=testServer.database.windows.net;Initial Catalog=testDB;User Id=testUser;Password=placeholder'))).rejects;
        expect(mssqlSpy).toHaveBeenCalledTimes(1);
    });
