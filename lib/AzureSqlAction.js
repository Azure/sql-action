...
class AzureSqlAction {
    ...
    _executeSqlFile(inputs) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield exec.exec(`"${sqlCmdPath}" -S ${inputs.serverName} -d ${inputs.parsedConnectionString.database} -U "${inputs.parsedConnectionString.userId}" -P "${inputs.parsedConnectionString.password}" -i "${inputs.sqlFile}" ${inputs.additionalArguments}`, [], {
                "windowsVerbatimArguments": true
            });
