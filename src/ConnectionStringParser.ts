...
const connectionStringTester = /^[;\s]*([\w\s]+=(?:('[^']*(''[^']*)*')|("[^"]*(""[^"]*)*")|((?!['"])[^;]*)))(;[;\s]*([\w\s]+=(?:('[^']*(''[^']*)*')|("[^"]*(""[^"]*)*")|((?!['"])[^;]*))))*[;\s]*$/g
...
export class ConnectionStringParser {
    ...
    public static parseConnectionString(connectionString: string): SqlConnectionString {
        if (!ConnectionStringParser.validateConnectionString(connectionString)) {
        ...
        let result = connectionString.matchAll(connectionStringParserRegex);
    ...
    public static validateConnectionString(connectionString: string) {
        return connectionStringTester.test(connectionString);
    }
}